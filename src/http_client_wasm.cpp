...
static void ParseResponseHeaders(const string &raw_headers, HttpHeaders &headers) {
	idx_t start = 0;
...
static HttpResponse ExecuteWasmHttpRequest(HttpClient &client, const string &method, const string &url,
	...
	if (!result) {
		response.request_error = "WASM browser HTTP request failed; no result returned";
		return response;
	}
...
HttpClient::HttpClient(int64_t timeout_ms_p) : timeout_ms(timeout_ms_p > 0 ? timeout_ms_p : 300000) {}
...
HttpResponse HttpClient::Get(const string &url, const HttpHeaders &headers) {
	return ExecuteWasmHttpRequest(*this, "GET", url, headers, string(), string(), timeout_ms);
}

HttpResponse HttpClient::Post(const string &url, const HttpHeaders &headers, const string &body,
                              const string &content_type) {
	return ExecuteWasmHttpRequest(*this, "POST", url, headers, body, content_type, timeout_ms);
}

HttpResponse HttpClient::Put(const string &url, const HttpHeaders &headers, const string &body,
                             const string &content_type) {
	return ExecuteWasmHttpRequest(*this, "PUT", url, headers, body, content_type, timeout_ms);
}

HttpResponse HttpClient::PostStream(const string &url, const HttpHeaders &headers, const string &body,
                                    const string &content_type,
                                    const std::function<bool(const_data_ptr_t data, idx_t data_length)> &receiver,
                                    bool disconnect_after_response) {
	(void)disconnect_after_response;
	auto started_at = std::chrono::steady_clock::now();
	auto response = Post(url, headers, body, content_type);
	if (!response.body.empty()) {
		response.first_byte_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
		                             std::chrono::steady_clock::now() - started_at)
		                             .count();
		response.streamed_chunks = 1;
		response.streamed_bytes = response.body.size();
		if (!receiver(const_data_ptr_cast(response.body.data()), response.body.size())) {
			Stop();
		}
	}
