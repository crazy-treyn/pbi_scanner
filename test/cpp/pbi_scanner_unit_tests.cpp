...
// MS-BINXML SQL-MONEY (token 0x05) — SSAS / sx can emit scalar typed values from the
// BINXML grammar, not only the compact tokens observed in early tests.
...
static std::string DecodeHex(const std::string &hex) {
	if (hex.size() % 2 != 0) {
		throw InvalidInputException("hex input must contain an even number of digits");
	}
	std::string result;
	result.reserve(hex.size() / 2);
	for (idx_t i = 0; i < hex.size(); i += 2) {
		auto high = DecodeHexDigit(hex[i], "hex input contained a non-hex character");
		auto low = DecodeHexDigit(hex[i + 1], "hex input contained a non-hex character");
...
static std::string CoerceXmlTypeString(const std::string &raw_value, const std::string &kind_name) {
	auto kind = ParseCoercionKind(kind_name);
	auto value = CoerceXmlValueForTesting(raw_value, kind);
	return value.type().ToString();
}

static std::string CoerceXmlTextString(const std::string &raw_value, const std::string &kind_name) {
	auto kind = ParseCoercionKind(kind_name);
	auto value = CoerceXmlValueForTesting(raw_value, kind);
	if (value.IsNull()) {
		return "<NULL>";
...
TEST_CASE("XMLA chunked parse", "[xmla][smoke]") {
	auto parsed = ParseXmlaChunksForTesting(
	    {"<?xml version=\"1.0\" encoding=\"utf-8\"?><root><schema "
	     "xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\"><xsd:schema><xsd:complexType "
	     "name=\"row\"><xsd:sequence><xsd:element name=\"Rate\" type=\"xsd:double\" "
	     "/></xsd:sequence></xsd:complexType></xsd:schema></schema><row><Rate>1.125E2</Ra",
	     "te></row></root>"});
	REQUIRE(RequireSingleDouble(parsed) == Approx(112.5));
}

TEST_CASE("BINXML double parse", "[xmla]") {
	REQUIRE(RequireSingleDouble(ParseBinXmlForTesting(DecodeHex(kBinXmlLegacyRate112_5))) == Approx(112.5));
	REQUIRE(RequireSingleDouble(ParseBinXmlForTesting(DecodeHex(kBinXmlCompactRate112_5))) == Approx(112.5));
	REQUIRE(RequireSingleDouble(ParseBinXmlForTesting(DecodeHex(kBinXmlSqlMoneyRate112_5))) == Approx(112.5));
}

TEST_CASE("BINXML first text", "[xmla]") {
	auto short_text = RequireSingleValue(ParseBinXmlForTesting(DecodeHex(kBinXmlShortFirstText)));
	REQUIRE(short_text.ToString().size() == 4);

	auto money_text = RequireSingleValue(ParseBinXmlForTesting(DecodeHex(kBinXmlSqlMoneyRate112_5)));
...
TEST_CASE("Streaming SX double parse", "[xmla]") {
	...
	REQUIRE(RequireSingleDouble(ParseStreamingSxRowsForTesting(decoded, 1)) == Approx(112.5));
	REQUIRE(RequireSingleDouble(ParseStreamingSxRowsForTesting(decoded, 7)) == Approx(112.5));
	REQUIRE(RequireSingleDouble(ParseStreamingSxRowsForTesting(decoded_money, 3)) == Approx(112.5));
}
...
TEST_CASE("DAX schema probe", "[dax_probe]") {
	REQUIRE(BuildDaxSchemaProbeForTesting("EVALUATE 'Fact Allocation'", 100) == "EVALUATE TOPN(100, 'Fact Allocation')");
	REQUIRE(BuildDaxSchemaProbeForTesting("EVALUATE ROW(\"x\", 1) ORDER BY [x]", 10) ==
	        "EVALUATE TOPN(10, ROW(\"x\", 1))");

	auto summarizing = StringUtil::Replace(
	    BuildDaxSchemaProbeForTesting(
	        "DEFINE MEASURE T[m] = 1 EVALUATE SUMMARIZECOLUMNS(T[c], \"m\", [m])", 5),
	    "\n", "|");
	REQUIRE(summarizing == "DEFINE MEASURE T[m] = 1 |EVALUATE TOPN(5, SUMMARIZECOLUMNS(T[c], \"m\", [m]))");

	REQUIRE(BuildDaxSchemaProbeForTesting("EVALUATE ROW(\"text\", \"ORDER BY ignored\")", 3) ==
	        "EVALUATE TOPN(3, ROW(\"text\", \"ORDER BY ignored\"))");
	REQUIRE(BuildDaxSchemaProbeForTesting("EVALUATE VAR X = ROW(\"x\", 1) RETURN X", 100) ==
	        "EVALUATE VAR X = ROW(\"x\", 1) RETURN X");
	REQUIRE(BuildDaxSchemaProbeForTesting("EVALUATE ROW(\"x\", 1)", 0) == "EVALUATE ROW(\"x\", 1)");
}
...
TEST_CASE("XML coercion type", "[xmla]") {
	REQUIRE(CoerceXmlTypeString("1899-12-30T12:34:56", "TIMESTAMP") == "TIMESTAMP");
	REQUIRE(CoerceXmlTypeString("1899-12-30T12:34:56+00:00", "TIMESTAMP_TZ") == "TIMESTAMP WITH TIME ZONE");
...
TEST_CASE("XML coercion text", "[xmla]") {
	...
	REQUIRE(CoerceXmlTextString("45292.25", "TIMESTAMP") == "2024-01-01 06:00:00");
}

TEST_CASE("Effective execution transport", "[xmla]") {
	REQUIRE(EffectiveExecutionTransportForTesting("EVALUATE INFO.VIEW.TABLES()") == "sx_xpress");
}
